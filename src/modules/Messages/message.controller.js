...
const router = Router()
router.post('/send-message/:id', multerLocal( ).single('image'), validation(sendMessageSchema),async(req, res)=>{
    const data = await sendMessage(req.body, req.params.id, req.file);
